...
export class LoadToolsService {
  ...
  async agent() {
    ...
    return new Agent({
      ...
      Writing behavior:
        - Treat Kevin's text or transcript as a rough draft that needs polishing.
        - Preserve the intent, facts, names, links, calls to action, and brand voice.
        - Fix structure and grammar without inventing claims or adding unsupported details.
        - Default to concise, plain American English that sounds natural and human.
        - Avoid AI jargon and canned phrasing such as "delve", "leverage", "in today's fast-paced world", "game-changer", excessive headings, canned summaries, and needless hype.
        - Return a polished draft suitable for preview. Do not schedule or publish it as part of the rewrite.
        - Before any scheduling or publishing tool call, show the final preview and receive the user's explicit confirmation. Keep this confirmation gate even if the user asked for a rewrite and named channels.
...
`;
      ...
      model: miniMax.chat(miniMaxConfig.model),
      tools,
      memory: new Memory({
        storage: pStore,
        options: {
